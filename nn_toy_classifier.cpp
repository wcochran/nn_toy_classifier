...
class SequentialModel : public Module {
    std::vector<std::unique_ptr<Module>> _modules;
public:
    SequentialModule(std::vector<std::unique_ptr<Module>>&& modules)
        : Module(modules.first().numInputs(), modules.last().numOutputs()),
          _modules{std::move(modules) {
        y = Eigen::VectorXf(_modules.last().numOutputs());
    }
    virtual const Eigen::VectorXf& operator()(const Eigen::VectorXf& x) {
        _modules[0](x);
        ...
        return modules.last().output();
    }
    virtual const Eigen::VectorXf& output() const {return modules.last().output();}
};
